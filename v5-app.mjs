...
function bind(){
  ...
  bindDnD();bindNavigatorDnD();bindLibraryExtras();bindVisualEditors();bindDeepAudit();bindAssetManager();bindSmartGuides();bindMyBlocksManager();bindBulkActions();
...
async function boot(){syncResponsivePanels();let p=await dbGet(DB_KEY);if(!p){const legacy=await dbGet(LEGACY_DB_KEY);p=migrateProject(legacy||defaultProject());upgradeLegacyContent(p);await dbPut(DB_KEY,p)}else{p=migrateProject(p);upgradeLegacyContent(p)}state.project=p;state.project.assets||=[];state.project.presets||=[];$('#uiLanguage').value=p.uiLang||'ru';setRender(renderAll);bind();renderAll();persist()}
boot();
