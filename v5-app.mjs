...
function enhanceUi(){enhanceLibraries();renderVisualEditors();if(state.activeRight==='site'){renderDeepAudit();renderAssetManager()}localizeUi()}
...
async function boot(){syncResponsivePanels();let p=await dbGet(DB_KEY);if(!p){const legacy=await dbGet(LEGACY_DB_KEY);p=migrateProject(legacy||defaultProject());upgradeLegacyContent(p);await dbPut(DB_KEY,p)}else{p=migrateProject(p);upgradeLegacyContent(p)}state.project=p;state.project.assets||=[];$('#uiLanguage').value=p.uiLang||'ru';setRender(renderAll);bind();renderAll();persist()}
boot();
