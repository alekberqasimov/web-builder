import {SCHEMA_VERSION,uid,clone,slugify,defaultSeo,baseTheme,block,container,makeHtml} from './v5-core.mjs';
import {preset} from './v5-presets.mjs';
...
export function duplicateNode(block0,nodeId){const found=findNodeWithParent(block0.root,nodeId);if(!found||!found.parent)return null;const c=clone(found.node);walk(c,n=>n.id=uid('el'));const i=found.parent.children.findIndex(x=>x.id===nodeId);found.parent.children.splice(i+1,0,c);return c}
