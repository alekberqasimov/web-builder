...
export function validateProjectInput(input){
 ...
 while(stack.length){const {v,depth}=stack.pop();if(!v||typeof v!=='object')continue;if(depth>80||++count>100000)throw new Error('Project exceeds supported size or nesting.');for(const k of Object.keys(v)){if(k==='id'&&typeof v[k]==='string'&&/[\s"'<>\\]/.test(v[k]))throw new Error('Unsafe object identifier.');if(['__proto__','prototype','constructor'].includes(k))throw new Error('Unsafe project property.');stack.push({v:v[k],depth:depth+1})}}
