...
async function realMouseDrag(page,source,target,{targetY=.7}={}){
  const sb=await stableBox(source),tb=await stableBox(target);
  ...
  await page.mouse.move(tx,ty,{steps:12});await wait(80);await page.mouse.up();await wait(180);
}
...
async function touchSuite(){
  ...
  assert.ok(nodeId&&targetId);const source=page.locator(`#canvas [data-node-id="${nodeId}"]`),target=page.locator(`#canvas [data-node-id="${targetId}"]`),sb=await stableBox(source),tb=await stableBox(target);
