...
try{
  ...
  const waValue=page.locator('#elementInspector [data-fx-social-value][data-i="0"]');
  await waValue.fill('+994 50 123 45 67');
  await waValue.blur();
  ...
  const waMessage=page.locator('#elementInspector [data-fx-social-message][data-i="0"]');
  await waMessage.fill('Salam Premium');
  await waMessage.blur();
  ...
  const tgValue=page.locator('#elementInspector [data-fx-social-value][data-i="1"]');
  await tgValue.fill('brand_support');
  await tgValue.blur();
