...
try{
  ...
  const exported=await page.evaluate(async()=>{
    const seo=await import('./v6-seo.mjs');
    const model=await import('./v5-model.mjs');
    const exp=await import('./v5-export.mjs');
    const runtime=await import('./v5-runtime.mjs');
    const project=runtime.state.project,page=project.pages.find(p=>p.id===project.currentPageId)||project.pages[0];
    page.seo.title='Production SEO Test';page.seo.description='Unique production description.';
    return{html:exp.exportedDocument(project,page),robots:seo.robotsText(project),sitemap:seo.sitemapXml(project),url:seo.publicPageUrl(project,page),issues:seo.seoAuditIssues(project)};
  });
