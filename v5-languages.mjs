...
export function defaultSiteLanguages(){
  return [
    {code:'ru',label:'Русский',default:true},
    {code:'az',label:'Azərbaycan',default:false},
    {code:'en',label:'English',default:false}
  ];
}

export function siteLanguages(project){
  const list=Array.isArray(project?.siteLanguages)?project.siteLanguages:[];
  return list.filter(x=>x&&normalizeLanguageCode(x.code)).map(x=>({code:normalizeLanguageCode(x.code),label:String(x.label||languageLabel(x.code)),default:!!x.default}));
}

export function defaultSiteLanguage(project){
  const list=siteLanguages(project);
  return (list.find(x=>x.default)||list[0]||{code:'ru'}).code;
}
...
export function ensureSiteLanguageConfig(project){
  ...
  if(!list.length){
    const seen=[];
    for(const p of project.pages||[]){const c=normalizeLanguageCode(p.lang||'');if(c&&!seen.includes(c))seen.push(c)}
...
export function pagesForLanguage(project,lang){
  const code=normalizeLanguageCode(lang||project?.activeSiteLang||defaultSiteLanguage(project));
  return (project?.pages||[]).filter(p=>normalizeLanguageCode(p.lang)===code);
}

export function linkedPage(project,pageOrId,lang){
  const page=typeof pageOrId==='string'?(project?.pages||[]).find(p=>p.id===pageOrId):pageOrId;
  if(!page)return null;
  const code=normalizeLanguageCode(lang);
  return (project?.pages||[]).find(p=>p.groupId===page.groupId&&normalizeLanguageCode(p.lang)===code)||null;
}

export function homePageForLanguage(project,lang){
  const list=pagesForLanguage(project,lang);
  return list.find(p=>p.home)||list[0]||null;
}

export function resolvePageForLanguage(project,targetPage,currentLang){
  if(!targetPage)return null;
  return linkedPage(project,targetPage,currentLang)||targetPage;
}

export function switchProjectLanguage(project,lang,currentPageId=''){
  ensureSiteLanguageConfig(project);
  const code=normalizeLanguageCode(lang);
  if(!project.siteLanguages.some(x=>x.code===code))return null;
  const current=(project.pages||[]).find(p=>p.id===(currentPageId||project.currentPageId));
  const next=linkedPage(project,current,code)||homePageForLanguage(project,code);
  project.activeSiteLang=code;
  if(next)project.currentPageId=next.id;
  return next;
}

export function addSiteLanguage(project,code,label=''){
  ensureSiteLanguageConfig(project);
  const c=normalizeLanguageCode(code);
...
export function removeSiteLanguage(project,code,{deletePages=false}={}){
  ...
  project.siteLanguages=project.siteLanguages.filter(x=>x.code!==c);
  if(row.default&&project.siteLanguages[0])project.siteLanguages[0].default=true;
  if(project.activeSiteLang===c)project.activeSiteLang=defaultSiteLanguage(project);
  const activePages=pagesForLanguage(project,project.activeSiteLang);
  if(!project.pages.some(p=>p.id===project.currentPageId)&&activePages[0])project.currentPageId=activePages[0].id;
  return true;
}
