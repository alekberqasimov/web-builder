...
export function block(name='Section',root=container()){return{id:uid('block'),type:'section',name,anchor:'',globalRole:'',preset:'',contentWidth:'',style:{base:{background:'#fff',color:'#111827',paddingTop:'64px',paddingBottom:'64px',paddingLeft:'24px',paddingRight:'24px'},tablet:{paddingTop:'54px',paddingBottom:'54px',paddingLeft:'20px',paddingRight:'20px'},mobile:{paddingTop:'40px',paddingBottom:'40px',paddingLeft:'16px',paddingRight:'16px'},hover:{},focus:{}},root}}
...
export function makeImageTextItem(index=1){const image=makeImage('',`Image ${index}`),title=makeHeading(`Заголовок ${index}`,3),text=makeText('Краткое описание элемента.'),button=makeButton('Подробнее');Object.assign(image.style.base,{borderRadius:'14px',aspectRatio:'4 / 3'});Object.assign(title.style.base,{fontSize:'20px'});Object.assign(text.style.base,{color:'#667085'});const c=container([image,title,text,button],{gap:12});Object.assign(c.style.base,{padding:'16px',border:'1px solid #e4e7ec',borderRadius:'18px',background:'#ffffff',boxShadow:'0 10px 30px rgba(15,23,42,.07)'});Object.assign(c.style.hover,{transform:'translateY(-2px)',boxShadow:'0 16px 38px rgba(15,23,42,.10)'});c.props.repeatItem='imageText';return c}
export function makeImageTextGrid(count=6){const safe=Math.max(1,Math.min(500,Number(count)||1)),grid=container(Array.from({length:safe},(_,i)=>makeImageTextItem(i+1)),{gap:18});grid.props.repeatTemplate='imageText';grid.props.itemCount=safe;grid.style.base.display='grid';grid.style.base.gridTemplateColumns='repeat(3,minmax(0,1fr))';grid.style.tablet.gridTemplateColumns='repeat(2,minmax(0,1fr))';grid.style.mobile.gridTemplateColumns='1fr';return grid}
