...
const templates = {
  ...
  cards:{icon:'▦',category:'content',html:`<div class="kicker" contenteditable="true">КАРТОЧКИ</div><h2 contenteditable="true">Изображения с описанием</h2><div class="image-text-grid"><article class="image-text-card"><img class="editable-image" src="${placeholderSvg('IMAGE 1')}" alt="Карточка 1" data-default="true"><h3 contenteditable="true">Заголовок 1</h3><p contenteditable="true">Краткое описание элемента.</p></article><article class="image-text-card"><img class="editable-image" src="${placeholderSvg('IMAGE 2')}" alt="Карточка 2" data-default="true"><h3 contenteditable="true">Заголовок 2</h3><p contenteditable="true">Краткое описание элемента.</p></article><article class="image-text-card"><img class="editable-image" src="${placeholderSvg('IMAGE 3')}" alt="Карточка 3" data-default="true"><h3 contenteditable="true">Заголовок 3</h3><p contenteditable="true">Краткое описание элемента.</p></article></div>`,style:{bg:'#ffffff',color:'#111827',padding:74,width:1120,align:'left'}},
...
function updateInspector(){
  ...
  $('#sectionType').value=blockName(section.type);$('#backgroundColor').value=toHex(section.style.bg);$('#backgroundText').value=section.style.bg;$('#textColor').value=toHex(section.style.color);$('#textColorText').value=section.style.color;$('#paddingRange').value=section.style.padding;$('#paddingValue').value=`${section.style.padding} px`;$('#contentWidth').value=String(section.style.width);$('#textAlign').value=section.style.align;updateRepeatInspector(section);updateImageInspector(section);
}
function repeatableInfo(section){
  const box=document.createElement('div');box.innerHTML=section.html;
  if(section.type==='gallery')return{box,container:box.querySelector('.gallery-grid'),selector:':scope > .editable-image'};
  if(section.type==='cards')return{box,container:box.querySelector('.image-text-grid'),selector:':scope > .image-text-card'};
  return null;
}
function updateRepeatInspector(section){const info=repeatableInfo(section);$('#repeatControls').classList.toggle('hidden',!info?.container);if(info?.container)$('#repeatCount').value=info.container.querySelectorAll(info.selector).length}
function repeatItem(type,index){
  if(type==='gallery'){const image=document.createElement('img');image.className='editable-image';image.src=placeholderSvg(`IMAGE ${index}`);image.alt=`Gallery ${index}`;image.dataset.default='true';return image}
  const card=document.createElement('article');card.className='image-text-card';card.innerHTML=`<img class="editable-image" src="${placeholderSvg(`IMAGE ${index}`)}" alt="Card ${index}" data-default="true"><h3 contenteditable="true">Title ${index}</h3><p contenteditable="true">Short item description.</p>`;return card;
}
function setRepeatCount(){
  const section=findSection(state.selected),info=section&&repeatableInfo(section);if(!info?.container)return;
  const requested=Math.max(1,Math.min(500,Math.round(Number($('#repeatCount').value)||1))),items=[...info.container.querySelectorAll(info.selector)];
  snapshot();while(items.length>requested)items.pop().remove();while(items.length<requested){const item=repeatItem(section.type,items.length+1);info.container.appendChild(item);items.push(item)}
  section.html=info.box.innerHTML;state.selectedImage=Math.min(state.selectedImage,requested-1);$('#repeatCount').value=requested;commit();renderCanvas();
}
...
function mutateImageLive(mutator){const section=findSection(state.selected);if(!section)return;const box=document.createElement('div');box.innerHTML=section.html;const image=box.querySelectorAll('.editable-image')[state.selectedImage];if(!image)return;mutator(image);section.html=box.innerHTML;const live=document.querySelector(`[data-id="${section.id}"]`)?.querySelectorAll('.editable-image')[state.selectedImage];if(live)mutator(live);commit()}
...
function dropIndexFromY(y){const elements=$$('.site-section:not(.dragging)');for(let i=0;i<elements.length;i++){const rect=elements[i].getBoundingClientRect();if(y<rect.top+rect.height/2)return i}return elements.length}
...
canvas.ondrop=e=>{e.preventDefault();const index=dropIndexFromY(e.clientY);removeMarkers();const type=e.dataTransfer.getData('template'),id=e.dataTransfer.getData('section');if(type)return addSection(type,index);if(id){const sections=currentSections(),old=sections.findIndex(s=>s.id===id);if(old<0)return;snapshot();const [item]=sections.splice(old,1);sections.splice(Math.max(0,Math.min(index,sections.length)),0,item);state.selected=id;commit();renderCanvas()}};
...
function preview(pageId=currentPage().id){
  const page=state.project.pages.find(item=>item.id===pageId)||currentPage();
  if(!page.sections.length)return notify(tr('empty'));
  $('#previewPageSelect').innerHTML=state.project.pages.map(item=>`<option value="${item.id}">${escapeHtml(item.name)} · ${item.lang.toUpperCase()}</option>`).join('');
  $('#previewPageSelect').value=page.id;
  updatePreviewFrame(page.id);
  if(!$('#previewDialog').open)$('#previewDialog').showModal();
}
function updatePreviewFrame(pageId){
  const page=state.project.pages.find(item=>item.id===pageId)||currentPage(),iframe=$('#previewFrame');
  iframe.srcdoc=exportedDocument(page);
  iframe.onload=()=>{
    try{iframe.contentDocument.querySelectorAll('a[href]').forEach(link=>link.addEventListener('click',event=>{const target=state.project.pages.find(item=>pageFile(item)===link.getAttribute('href'));if(target){event.preventDefault();$('#previewPageSelect').value=target.id;updatePreviewFrame(target.id)}}))}catch(error){console.warn('Preview navigation unavailable',error)}
  };
}
...
function bindUi(){
  ...
  $('#uiLanguage').onchange=e=>{state.uiLang=e.target.value;applyI18n();commit()};$('#undoBtn').onclick=undo;$('#redoBtn').onclick=redo;$('#previewBtn').onclick=()=>preview();$('#downloadBtn').onclick=downloadSite;$('#leftToggleBtn').onclick=()=>togglePanel('left');$('#rightToggleBtn').onclick=()=>togglePanel('right');
  $('#closePreviewBtn').onclick=()=>$('#previewDialog').close();$('#previewPageSelect').onchange=e=>updatePreviewFrame(e.target.value);$$('[data-preview-device]').forEach(button=>button.onclick=()=>{$$('[data-preview-device]').forEach(item=>item.classList.toggle('active',item===button));$('#previewFrame').className=button.dataset.previewDevice});
